...
def edit_distance(
    ...
    for i in range(1, m + 1):
        ...
        for j in range(1, n + 1):
            if type(seq1[i - 1])==list:
                seq1_candidate = set(seq1[i - 1])
            else:
                seq1_candidate = set([seq1[i - 1]])
            if type(seq2[j - 1])==list:
                seq2_candidate = set(seq2[j - 1])
            else:
                seq2_candidate = set([seq2[j - 1]])

            #cost = 0 if test(seq1_candidate, seq2_candidate) else 1
            cost = 0 if len(seq1_candidate.intersection(seq2_candidate))>0 else 1
